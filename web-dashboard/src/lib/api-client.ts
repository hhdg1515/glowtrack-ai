 ...
 *
 * Mock 模式说明：
 * 设置环境变量 NEXT_PUBLIC_USE_MOCK=true 可以启用 mock 数据模式
 * 默认使用真实 API（需要后端服务运行）
 */
...
import { mockPatientsApi, mockTreatmentsApi } from './mock-api-client'
...
export const patientsApi = USE_MOCK
  ? mockPatientsApi
  : {
      /**
       * 获取诊所的所有患者
...
export const treatmentsApi = USE_MOCK
  ? mockTreatmentsApi
  : {
      /**
       * 获取患者的所有治疗记录
